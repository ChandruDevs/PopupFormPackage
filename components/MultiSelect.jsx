...
export default function MultiSelect({ item, FormData, setFormData}) {
    ...
    useEffect(()=>{
      ...
      formData[item.keyword] = selectedItems.map((items)=>{return items.value});
    ...
    const handleSelectedValues = (e)=>{
        ...
        setSelectedItems(formDropDownData().filter((value)=> selectedId.includes(value.id)))
    }
      ...
      <span 
        ...
        onClick={(e)=>{setShowDropdown(!showDropdown)}}>
         {  selectedItems.length > 0 ?
           selectedItems.map(
            (items)=>{return <span>{items.value}</span>}
            )
            :'Nothing Selected'}
      </span>

      {showDropdown && <div className='values'>
      <div className='search'>
        <input 
          placeholder='Search'
          onChange={searchItems}/>
        {selectedItems && selectedItems.length>0 && <span onClick={clearSelectedValues} className='clear'>Clear</span>}
      </div>
